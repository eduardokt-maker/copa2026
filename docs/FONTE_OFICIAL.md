...
- A consulta automatica de placares deve priorizar a API estruturada oficial da FIFA:
  `https://api.fifa.com/api/v3/calendar/matches?language=en&count=500&idCompetition=17&idSeason=285023`.
- A pagina publica da FIFA continua como verificacao visual e fonte oficial complementar:
...
- O backend tenta consultar a API estruturada da FIFA e a pagina oficial da FIFA, informando o status em `official_source`.
- Jogos encerrados e confirmados ficam gravados em `data/final_matches.json`.
- Placar publicado pela API FIFA sem status final pode ser mostrado como atualizacao ao vivo, mas nao entra em `data/final_matches.json` ate estar encerrado.
...
- Um jogo encerrado ja gravado nao deve ser sobrescrito por dado externo vazio, parcial ou sem placar final; dado final completo vindo da FIFA pode atualizar metadados e corrigir o banco definitivo.
...
- Quando a FIFA publicar ou alterar placar, data, horario ou estadio de uma fase futura, o backend deve ser a origem unica para atualizar os cards da tela Mata-mata e do Calendario Geral.
...
- `/api/live-sync`: mostra modo atual da rotina (`live`, `matchday` ou `routine`), intervalo de proxima consulta, contagem da API FIFA e ultimo status da fonte.
